<x-layouts.super-admin title="Edit Business">
    ...
    <div class="mb-6">
        <h2 class="text-xl font-bold text-slate-800">Edit Business</h2>
        <p class="text-sm text-slate-500 mt-0.5">Update business information and settings</p>
    </div>
    ...
    <form
        ...
        x-data="{
            ...
            logoPreview: {{ $business->logo ? "'/storage/" . $business->logo . "'" : 'null' }},
            submitting: false,
            errors: {},
            validate(field) {
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            >
                                @foreach(['cafe' => '☕ Cafe', 'hotel' => '🏨 Hotel', 'restaurant' => '🍽️ Restaurant', 'bar' => '🍺 Bar', 'other' => '🏢 Other'] as $val => $label)
                                    <option value="{{ $val }}" {{ old('type', $business->type) === $val ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('type')
                        ...
                        <div>
                            ...
                            <select
                                ...
                                class="w-full px-3 py-2.5 text-sm border border-slate-300 rounded-xl bg-white text-slate-800 focus:outline-none focus:ring-2 focus:border-amber-400 focus:ring-amber-200 transition-all"
                            >
                                @foreach(['pending' => 'Pending', 'active' => 'Active', 'inactive' => 'Inactive', 'suspended' => 'Suspended'] as $val => $label)
                                    <option value="{{ $val }}" {{ old('status', $business->status) === $val ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('status')
                                <p class="text-xs text-red-500 mt-1.5">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($subscriptionPlans as $plan)
                                <option value="{{ $plan->id }}" {{ old('subscription_plan_id', $business->subscription_plan_id) == $plan->id ? 'selected' : '' }}>
            ...
            <div class="space-y-6 lg:sticky lg:top-6">
                ...
                <x-ui.card title="Quick Guide">
                    <ul class="space-y-3 text-sm text-slate-600">
                        <li class="flex items-start gap-2">
                            ...
                            <span>Update business logo (optional).</span>
                        </li>
                        <li class="flex items-start gap-2">
                            ...
                            <span>Modify business name, type, and description.</span>
                        </li>
                        <li class="flex items-start gap-2">
                            ...
                            <span>Update business email and contact details.</span>
                        </li>
                        <li class="flex items-start gap-2">
                            ...
                            <span>Change subscription plan if needed.</span>
                        </li>
                    </ul>
                </x-ui.card>
                ...
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-500 space-y-1.5">
                    ...
                    <p><span class="font-medium text-red-600">Suspended</span> — temporarily blocked</p>
                </div>
                ...
                <div class="flex flex-col gap-3">
                    ...
                    >
                        ...
                        <span x-text="submitting ? 'Updating...' : 'Update Business'"></span>
                    </button>
